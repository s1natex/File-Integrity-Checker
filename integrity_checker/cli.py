...
def compute_hash(file_path: Path, algo: str) -> str | None:
...
def stable_entries_bytes(entries: dict) -> bytes:
    return json.dumps(entries, sort_keys=True, separators=(",", ":")).encode("utf-8")

def verify_signature(db: dict, key_env: str = "INTEGRITY_KEY") -> bool | None:
...
def save_db_atomic(db_path: Path, db: dict):
    tmp = None
    try:
        ...
        if key:
            algo = db.get("meta", {}).get("algo", "sha256")
            db["signature"] = hmac_sign(stable_entries_bytes(db["entries"]), key.encode("utf-8"), algo)
        tmp_fd, tmp_name = tempfile.mkstemp(prefix=".integrity.", dir=str(db_path.parent))
        tmp = Path(tmp_name)
...
def cmd_init(path: Path, db_path: Path, pattern: str, algo: str, follow_symlinks: bool, include_symlinks: bool):
    db = load_db(db_path)
    db["meta"]["algo"] = algo
...
def _check_one(p: Path, db: dict, algo: str, follow_symlinks: bool):
    cpath = canonical(p, follow_symlinks)
...
def cmd_check(path: Path, db_path: Path, pattern: str, json_out: bool, follow_symlinks: bool, include_symlinks: bool):
    db = load_db(db_path)
    sig_ok = verify_signature(db)
    if sig_ok is False:
        print("WARNING: Hash DB signature verification FAILED. DB may be tampered.", file=sys.stderr)
    ...
    for ep in db["entries"].keys():
        if str(ep).startswith(str(root)):
            if not Path(ep).exists():
                missing.append({"path": ep, "status": "missing", "message": "Previously tracked, now missing"})
    ...
    summary = {
        "total_checked": len([r for r in results if r["status"] in ("ok","modified","new","error")]),
        "ok": len([r for r in results if r["status"] == "ok"]),
        "modified": len(modified),
        "missing": len(missing_list),
        "new": len(new),
        "errors": len(errors),
        "db_signature_verified": (True if sig_ok else False) if sig_ok is not None else None,
    }
    ...
    if json_out:
        ...
        print(
            ...
            + ("" if summary["db_signature_verified"] is None else
               f" | DB signature ok={summary['db_signature_verified']}")
        )
...
def cmd_update(path: Path, db_path: Path, pattern: str, follow_symlinks: bool, include_symlinks: bool):
    db = load_db(db_path)
    algo = db.get("meta", {}).get("algo", "sha256")
    paths = iter_files(path, pattern, follow_symlinks)
    updated = 0
...
def build_parser():
    ...
    common.add_argument("--db", default=str(DEFAULT_DB), help=f"Path to hash DB (default: {DEFAULT_DB})")
    common.add_argument("--pattern", default=DEFAULT_PATTERN, help=f"Filename glob when PATH is a directory (default: {DEFAULT_PATTERN})")
    common.add_argument("--follow-symlinks", action="store_true", help="Resolve symlinks (default: off)")
    common.add_argument("--include-symlinks", action="store_true", help="Include symlinked files in scans (default: off)")

    p_init = sub.add_parser("init", parents=[common], help="Initialize hashes")
    p_init.add_argument("--algo", choices=SUPPORTED_ALGOS.keys(), default="sha256", help="Hash algorithm")
    ...
    p_update = sub.add_parser("update", parents=[common], help="Update stored hash(es)")
...
def main():
    ...
    if args.command == "init":
        cmd_init(path, db_path, args.pattern, args.algo, args.follow_symlinks, args.include_symlinks)
    elif args.command == "check":
        cmd_check(path, db_path, args.pattern, args.json, args.follow_symlinks, args.include_symlinks)
